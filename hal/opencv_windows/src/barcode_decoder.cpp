...
WindowsBarcodeDecoder::WindowsBarcodeDecoder() : frame_{} {
  scanner_.set_config(::zbar::ZBAR_NONE, ::zbar::ZBAR_CFG_ENABLE, 1);
}

ReturnResult<BarcodeInfo> WindowsBarcodeDecoder::GetValue(void) {
  ...
  if (decoded_objects_.count > WindowsBarcodeDecoder::MAX_SCAN_CODES) {
    decoded_objects_.count = WindowsBarcodeDecoder::MAX_SCAN_CODES;
  }

  // Clear return buffers
  for (int count = 0; count < WindowsBarcodeDecoder::MAX_SCAN_CODES; count++) {
...
ReturnResult<BarcodeInfo> WindowsBarcodeDecoder::GetMaxValue(void) {
  return -1;
}

ReturnResult<BarcodeInfo> WindowsBarcodeDecoder::GetMinValue(void) {
  return -1;
}

ReturnResult<void> WindowsBarcodeDecoder::SetValue(const VideoFrame frame) {
  frame_ = frame;
